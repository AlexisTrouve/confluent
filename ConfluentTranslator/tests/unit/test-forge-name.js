...
const { forgeProperName, stubForge } = require('../../src/core/translation/nameForge');
...
(async () => {
  ...
  let calls = 0;
  const fakeForge = async () => { calls++; return { confluent: 'silibuna', racines: ['sili', 'buna'], liaison: 'i', decompo: 'sil-i-buna' }; };
  ...
  check('forme retournée = celle du sous-agent', r1.confluent === 'silibuna');
  check('sous-agent appelé 1 fois', calls === 1);
  check('persisté au registre', ctx.registry.lookup('Cours-Devant')?.confluent === 'silibuna');
  ...
  check('2e appel → MÊME forme', r2.confluent === 'silibuna');
  ...
  let n = 0;
  const ctxFix = { ...ctxBase(), forgeFn: async () => (++n === 1 ? { confluent: 'tbima' } : { confluent: 'silibuna', racines: ['sili', 'buna'] }) };
  const rf = await forgeProperName({ nom_fr: 'Test-Repar', sens: 'x' }, ctxFix);
  check('1 reprise après rejet → forge réussie', rf.forged === true && rf.confluent === 'silibuna' && n === 2);
