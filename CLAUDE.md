...
**Système MULTI-ÈRE** : `proto` (primitif) / `ancien` (unifié) / **`mythologique`** (registre sacré). Toutes les briques (gate, morpho, outils) sont paramétrées par l'ère (`src/core/eras/eras.js`). Le **Confluent mythologique** est câblé dans tous les layers : registre haut de l'ancien (hérite tout + active y/é/è) avec une strate sacrée (cosmogonie des chants), une **forge de noms propres** (`forge_proper_name`), et un **streaming SSE** qui rend le travail de l'agent visible en direct. **→ Doctrine et direction complètes : `docs/langue/06-CONFLUENT-MYTHIQUE.md`** (à lire avant tout travail sur le mythique). **→ État des lieux complet (vocab forgé + ARMING + résultats sur vrais chants + journal) : `docs/langue/atelier/ETAT-DES-LIEUX-MYTHOLOGIQUE.md`.** Le vocab (114 sens natifs) et l'arming (carte des sens natifs) sont **FAITS** ; cap neuf = la **forme poétique** (Éluard).

> **⭐ Doctrine vocab** : enrichir le lexique **DEPUIS LE CANON** (la vision du monde Regard⚔Vide), **jamais en calquant le français**. DEUX mouvements : **(A) éclater les mots-tiroir** (1 forme surchargée → N sens natifs) et **(B) décliner les concepts solo** (1 mot plat que le canon rend riche → sous-sens, ex. `osi`=mort → 5 destins d'éclat). La traduction reste **auto** (l'IA décide), mais on l'**arme** : formes + propositions natives + **définitions**. **→ MÉTHODE complète (à lire avant tout travail vocab) : `docs/langue/atelier/METHODE-ENRICHISSEMENT.md`** ; doctrine : `docs/langue/06-CONFLUENT-MYTHIQUE.md` §3-4.
...
- `core/translation/conceptMap.js` — **ARMING** : carte compacte des sens natifs (groupée par champ `famille`) injectée dans le system prompt via `promptBuilder.loadBaseTemplate` → le LLM **emploie** les déclinaisons (osiieku « montée-en-étoile »…) par matching sémantique, là où le lookup mot-à-mot les ratait. Familles taguées par `scripts/_tag-familles.js`. La détection lexicale par phrase a été testée puis **abandonnée** (trop fragile : 50% recall, faux positifs).
...
Cap réel = **transcréer les chants du Livre de la Foi** dans le registre mythologique. **→ ÉTAT DES LIEUX COMPLET (couche mytho de A à Z : forge + arming + résultats sur vrais chants + ce qui reste) : `docs/langue/atelier/ETAT-DES-LIEUX-MYTHOLOGIQUE.md`** ; doctrine : `docs/langue/06-CONFLUENT-MYTHIQUE.md`.

1. ✅ **Diversifier le vocab** — **FAIT** : **~114 sens natifs** en **~20 familles**, deux mouvements — (A) éclater les mots-tiroir (~37 sens) + (B) **décliner les concepts solo** (61 déclinaisons : osi→5 morts, aita→5 ancêtres, le nom nuli⚔kinakova, lusak…). `ancien-confluent/lexique/32-sens-natifs.json` + strate sacrée `mythologique-confluent/lexique/02-sens-natifs-sacres.json`. Gabarit : verbe 5-CVCVC / nom 4-6 / mix. Audit 0/0/0/0. Cartos : `docs/langue/atelier/{METHODE-ENRICHISSEMENT,RECHERCHE-DECLINAISONS,PROCESS-DIVERSIFICATION,INVENTAIRE-MOTS-TIROIR,FIX-PROPOSITIONS}.md`.
2. ✅ **Armer la traduction** — **FAIT** : `lookup_concept` surface def+nuance ; ET **carte des sens natifs** (par `famille`) injectée dans le system prompt (`conceptMap.js`) → le LLM **emploie** les déclinaisons par matching sémantique. Prouvé live Opus, y compris sur de vrais chants (`vèluk`/`mevak`/`tonak`/`èva`).
3. 🔜 **CAP — la POÉTIQUE Éluard** : le rendu est sémantiquement juste mais **« lacking »** (verdict Alexi 2026-06-11) — le canon FR est écrit *à la Éluard*. Développer une **forme poétique** du Confluent (souffle, image, répétition, musique des sons/voyelles sacrées). **Lire d'abord** la doctrine existante : `civjdr/livre_de_la_foi/chants/traductions/CN/DOCTRINE_TRADUCTION_CN.md`.
4. **Grammaire** — devices natifs pour le benefactif (« à ta place ») et la copule définitionnelle (« X c'est Y »).
5. **Transcréer les chants** en volume + figer les noms forgés bénis en lexique versionné. *(2 arbitrages de vision ouverts : `ena` vs `èva` origine ultime ; consolidation `zakis`→`konu`.)*
